...
new #[Layout('layouts.application')] class extends Component
{
    ...
    public function mount(string $token): void
    {
        ...
        $this->application = HealthcareApplication::where('token', $token)
            ->first();
        ...
        if (ApplicationAccessSession::hasVerified($token)) {
            $this->redirect(route('application.healthcare.form', ['token' => $token]));
        }
    }
    ...
    public function verify(): void
    {
        ...
        $this->redirect(route('application.healthcare.form', ['token' => $this->token]));
    }
};
